                            ...
                            if (!Process.WaitForExit(halfTimeOut))
                            {
                                ...
                                try
                                {
                                        ...
                                        Interlocked.CompareExchange(ref s_clrMdLock, 1, 0) == 0) // Make sure we only attach to one process at a time.
                                    {
                                        try
                                        {
                                            using DataTarget dt = DataTarget.AttachToProcess(Process.Id, true); 
                                            ...
                                            if (runtime != null)
                                            {
                                                ...
                                                foreach (ClrThread thread in runtime.Threads.Where(t => t.IsAlive))
                                                {
                                                    ...
                                                    string isBackground = ((thread.State & ClrThreadState.TS_Background) == ClrThreadState.TS_Background) ? "[Background]" : "";
                                                    string apartmentModel = ((thread.State & ClrThreadState.TS_InMTA) == ClrThreadState.TS_InMTA) ? "[MTA]" :
                                                          ((thread.State & ClrThreadState.TS_InSTA) == ClrThreadState.TS_InSTA) ? "[STA]" :
                                                          "";
