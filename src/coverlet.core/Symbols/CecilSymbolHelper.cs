...
namespace Coverlet.Core.Symbols
{
  internal class CecilSymbolHelper : ICecilSymbolHelper
  {
    ...
    private readonly ConcurrentDictionary<string, HashSet<string>> _backingFieldCache = new();

    // Experimental: set COVERLET_EXPERIMENTAL_AUTOPROP_BACKING_FIELD_CACHE=1 to enable the
    // cached O(1) backing-field lookup instead of the default O(N) LINQ scan.
    // When the variable is absent or falsy the behaviour is identical to the current master branch.
    private static readonly bool s_useBackingFieldCache =
        IsExperimentalFlagSet("COVERLET_EXPERIMENTAL_AUTOPROP_BACKING_FIELD_CACHE");

    private static bool IsExperimentalFlagSet(string name)
    {
   ...
   */
    ...
    private bool SkipGeneratedBackingFieldAssignment(MethodDefinition methodDefinition, Instruction instruction)
    {
      /*
          ...
          ...
      */
      return s_useBackingFieldCache
          ? SkipGeneratedBackingFieldAssignmentCached(methodDefinition, instruction)
          : SkipGeneratedBackingFieldAssignmentDefault(methodDefinition, instruction);
    }

    /// <summary>
    /// Original O(N) implementation — used when the experimental cache flag is not set.
    /// Behaviour is identical to the master branch.
    /// </summary>
    private static bool SkipGeneratedBackingFieldAssignmentDefault(MethodDefinition methodDefinition, Instruction instruction)
    {
      IEnumerable<FieldDefinition> autogeneratedBackingFields = methodDefinition.DeclaringType.Fields.Where(x =>
          ...
          x.FullName.EndsWith("k__BackingField", StringComparison.Ordinal));
    ...
    /// <summary>
    /// Experimental O(1) implementation — enabled by setting the environment variable
    /// <c>COVERLET_EXPERIMENTAL_AUTOPROP_BACKING_FIELD_CACHE=1</c>.
    /// Caches the set of backing-field names per declaring type so the per-field LINQ scan
    /// is paid at most once per type across all constructor instructions.
    /// </summary>
    private bool SkipGeneratedBackingFieldAssignmentCached(MethodDefinition methodDefinition, Instruction instruction)
    {
      if (instruction.Next?.Next?.OpCode != OpCodes.Stfld ||
          instruction.Next?.Next?.Operand is not FieldReference fr)
        return false;

      // The 2-arg GetOrAdd overload is used instead of the 3-arg (state-param) overload to
      // stay compatible with .NET Standard 2.0 (the 3-arg overload requires .NET Standard 2.1+).
      TypeDefinition declaringType = methodDefinition.DeclaringType;
      string cacheKey = $"{declaringType.Module.Mvid}:{declaringType.FullName}";
      HashSet<string> backingFields = _backingFieldCache.GetOrAdd(
          cacheKey,
          _ =>
          {
            var set = new HashSet<string>(StringComparer.Ordinal);
            foreach (FieldDefinition f in declaringType.Fields)
            {
              if (f.FullName.EndsWith("k__BackingField", StringComparison.Ordinal) &&
                  f.CustomAttributes.Any(ca => ca.AttributeType.FullName == typeof(CompilerGeneratedAttribute).FullName))
              {
                set.Add(f.FullName);
              }
            }
      ...
      return backingFields.Contains(fr.FullName); // O(1) lookup instead of O(N) linear scan
    }
