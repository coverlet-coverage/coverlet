...
namespace Coverlet.MSbuild.Tasks
{
  internal class ReportWriter
  {
    ...
    public ReportWriter(string coverletMultiTargetFrameworksCurrentTFM, string directory, string output,
                        IReporter reporter, IFileSystem fileSystem, ILogger logger, CoverageResult result, ISourceRootTranslator sourceRootTranslator)
        => (_coverletMultiTargetFrameworksCurrentTFM, _directory, _output, _reporter, _fileSystem, _logger, _result, _sourceRootTranslator) =
            (coverletMultiTargetFrameworksCurrentTFM, directory, output, reporter, fileSystem, logger, result, sourceRootTranslator);
    ...
    public string WriteReport()
    {
      ...
      _logger.LogInformation($"  Generating report '{report}'", true);
