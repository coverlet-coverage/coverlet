    ...
    public class CoverageTests
    {
        ...
        public void ExcludeFilteredTypes()
        {
            ...
            try
            {
                RemoteExecutor.Invoke(async pathSerialize =>
                {
                    CoveragePrepareResult coveragePrepareResult = await TestInstrumentationHelper.Run<ExcludeOuterTypes>(instance =>
                    {
                        Assert.Equal(42, instance.Run());
                        return Task.CompletedTask;
                    },
                    excludeFilter: moduleFileName => new string[] { $"[{Path.GetFileNameWithoutExtension(moduleFileName)}*]*ExcludeOuterTypes" },
                    persistPrepareResultToFile: pathSerialize);
