...
namespace Coverlet.Integration.Tests
{
    public class DeterministicBuild : BaseTest, IDisposable
    {
        ...
        public void Msbuild()
        {
            ...
            string sourceRootMappingFilePath = Path.Combine(_testProjectPath, "bin", _buildConfiguration, _testProjectTfm!, $"CoverletSourceRootsMapping_coverletsamplelib.integration.template");
        ...
        public void Collectors()
        {
            ...
            DotnetCli($"build -c {_buildConfiguration} /p:DeterministicSourcePaths=true", out string standardOutput, out string _, _testProjectPath);
            Assert.Contains("Build succeeded.", standardOutput);
            string sourceRootMappingFilePath = Path.Combine(_testProjectPath, "bin", GetAssemblyBuildConfiguration().ToString(), _testProjectTfm!, $"CoverletSourceRootsMapping_coverletsamplelib.integration.template");
            Assert.True(File.Exists(sourceRootMappingFilePath), sourceRootMappingFilePath);
